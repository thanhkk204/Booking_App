...
export const updateDoctor = async (req, res) => {
  ...
  try {
    const doctorUpdated = await DoctorModule.findByIdAndUpdate(
      { _id: id },
      { $set: req.body },
      { new: true }
    )
...
export const findAllDoctors = async (req, res) => {
  ...
  try {
    if (query) {
      ...
      doctors = await DoctorModule.find({ isApproved: "approved" }).select(
        "-password"
      )
    }
...
export const getDoctorProfile = async (req, res) => {
  ...
  try {
    ...
    const appointments = await BookingModule.find({
      doctor: doctorId,
    }).populate("user")
