...
export const MoMopayment = async (req, respond) => {
  ...
  var extraData = btoa(JSON.stringify({ bookingId: savedBooking._id }))
  var paymentCode =
...
export const updateBooking = async (req, res) => {
  ...
  try {
    if (!bookingId) {
      return res
        .status(401)
        .json({ success: false, massage: "Can't get bookingId" })
    }
    ...
    res
      .status(201)
      .json({ success: true, massage: "Your appointment is reserved" })
  } catch (error) {
    res
      .status(501)
      .json({ success: false, massage: "Something was wrong on server" })
  }
}
